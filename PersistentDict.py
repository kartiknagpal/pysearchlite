...
if __name__ == '__main__':
    ...
    with PersistentDict(os.path.join(os.path.dirname(sys.argv[0]),'demo.json'), \
        'c', format='json') as d:
        print(d, 'start')
        #d['abc'] = []
        for i in range(1,10):
            try:
                d['abc'] = d['abc'].append(i)
            except KeyError:
                d['abc'] = [1]
    ...
    with open(os.path.join(os.path.dirname(sys.argv[0]),'demo.json'), 'rb') as f:
        print(f.read())
